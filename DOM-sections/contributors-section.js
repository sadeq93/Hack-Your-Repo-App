...
export class contributors {
  ...
  calculateNumOfButtons(numOfContributors) {
    if (numOfContributors <= 5) {
      ...
      const numberOfContributors = Math.floor(numOfContributors / 5);
  ...
  async createPagesButtonsContainer(e) {
    //e.Select
    const data = await this.repoData;
    const contributorsData = await axios.get(
      data[e.target.value].contributors_url
    );
  ...
  async runOnChange(e) {
    if (
      this.contributorsSection.firstElementChild === this.contributorsMessage
    ) {
    ...
    if (
      this.contributorsSection.firstElementChild === this.contributorsContainer
    ) {
